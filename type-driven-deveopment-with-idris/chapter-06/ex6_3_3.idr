...
parseBySchema schema x = do
  stuff <- parsePrefix schema x
  case stuff of 
    (res, "") => Just res
    _         => Nothing
...
getAll store = vectToString 0 (items store)
  where
    vectToString : Nat -> Vect n (SchemaType (schema store)) -> String
    vectToString _ [] = ""
    vectToString nat (x :: xs) = show nat ++ ": " ++ display x ++ "\n" ++ vectToString (nat + 1) xs
